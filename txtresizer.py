...
class AutoResizedText(Frame):
    def __init__(
        self, parent, width=0, height=0, family=None, size=None, *args, **kwargs
    ):
        ...
        if family != None and size != None:
            self._font = tkinter.font.Font(family=family, size=size)
        else:
            self._font = tkinter.font.Font(family=self._textarea.cget("font"))
        ...
        list_of_bind_tags.insert(
            list_of_bind_tags.index("Text"), self._autoresize_text_tag
        )

        self._textarea.bindtags(tuple(list_of_bind_tags))
        self._textarea.bind_class(
            self._autoresize_text_tag, "<KeyPress>", self._on_keypress
        )
    ...
    def _on_keypress(self, event):
        ...
        # We check whether it is a punctuation or normal key
            ...
            new_text = self._insert_character_into_message(
                old_text, self._textarea.index(INSERT), new_char
            )
    ...
    def _fit_to_size_of_text(self, text):
        ...
        widget_width += (
            2 * self._textarea["bd"]
            + 2 * self._textarea["padx"]
            + self._textarea["insertwidth"]
        )
