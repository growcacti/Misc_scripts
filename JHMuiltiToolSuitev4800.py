    ...
    def search_for_pattern(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"An error occurred: {e}")
             ...
             print(f"Unicode error while reading the file: {file_path}")
...
class Concatinate_Text:
    ...
    def clearall(self):
        self.txtout.delete("1.0", tk.END)
        self.listbox.delete(0, tk.END)
class Process_Text_Csv:
    def __init__(self, parent):
        self.parent = parent
        self.parent.title("Text File Processor")
        self.create_widgets()
    
    def create_widgets(self):
        select_button = tk.Button(self.parent, text="Select Directory and Process Files", command=self.select_directory)
        select_button.grid(row=1, column=0)
    
    def process_files(self, directory, csv_filename):
        try:
                ...
                for filename in os.listdir(directory):
                    if filename.endswith('.txt'):
                        ...
                        with open(filepath, 'r') as text_file:
                            try:
                                lines = [next(text_file) for _ in range(20)]
                            except StopIteration:
                                continue  # Skip files with less than 20 lines
    ...
    def select_directory(self):
        ...
        if directory:
             epoch_time = int(time.time())
             csv_filename = f"{epoch_time}_output.csv"  
             self.process_files(directory, csv_filename)
...
# If you want to append to an existing CSV file, open it with mode 'a' instead of 'w'
# Create the main window and the TextFileProcessor instance


class CSVColumnDisplayApp:
    def __init__(self, parent):
        self.parent = parent
     
        
        self.listbox_frame = tk.Frame(self.parent)
        self.listbox_frame.grid(row=5, column=1)
        
        self.listboxes = []
        self.column_labels = []
        
        self.load_button = tk.Button(self.parent, text="Load and Process File", command=self.process_and_load_file)
        self.load_button.grid(row=2, column=1)
        
        self.entry = tk.Entry(self.parent, bd=7)
        self.entry.grid(row=2, column=2)
        
        tk.Label(self.parent, text="Number of lines to skip:").grid(row=1, column=0)
        self.skip_lines_entry = tk.Entry(self.parent, bd=7)
        self.skip_lines_entry.grid(row=1, column=1)
        self.skip_lines_entry.insert(0, "33")  # Default value of 33 lines to skip
    def process_and_load_file(self):
        try:
            lines_to_skip = int(self.skip_lines_entry.get())
        except ValueError:
        ...
        with open(file_path, 'r') as text_file, open(csv_file_path, 'w', newline='') as csv_file:
            ...
            for line in text_lines:
                ...
                print("csv created")# Assuming space-separated values
...
if __name__ == '__main__':
    ...
    replace_save = Muilti_Find_Replace(mfr)
    ...
    logfrm = tk.Frame(notebook)
    notebook.add(logfrm, text="autolog create from csv")
    test_report_to_log = Process_Text_Csv(logfrm)

    skiplinefrm = tk.Frame(notebook)
    notebook.add(skiplinefrm, text="set skiplines before csv")
