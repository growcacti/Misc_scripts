import os
from tkinter import Tk, Frame, Label, Entry, Button, Text, filedialog, Scrollbar, END, Checkbutton, IntVar
...
class WordCounterApp:
    def __init__(self, root):
        ...
        self.all_words_var = IntVar()
        self.all_words_check = Checkbutton(self.frame, text="Count all words and characters", variable=self.all_words_var)
        self.all_words_check.grid(row=1, column=0, columnspan=2, padx=5, pady=5)
    ...
    def count_occurrences(self):
        ...
        count_all_words = self.all_words_var.get()

        if not search_term and not count_all_words:
            self.text_area.insert(END, "Please enter a word or number to count or select 'Count all words and characters'.\n")
            return

        self.text_area.insert(END, f"Counting occurrences...\n")
        ...
        if count_all_words:
            self.text_area.insert(END, f"Results for all words and characters:\n")
        else:
        ...
        for file_path, word_count, char_count, line_count in results:
            ...
            if count_all_words:
                self.text_area.insert(END, f"  {word_count} words\n")
            else:
