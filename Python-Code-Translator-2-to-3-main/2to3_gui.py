...
def process_single_file(self, fileName):
    """
    A method that opens a single file and processes it.
    """
    detected_encoding = detect_encoding(fileName)

    print(f"Processing {fileName} ({detected_encoding})")

    with open(fileName, 'r+', encoding=detected_encoding) as fileStream:
...
def open_file_dialog():
    ...
    file_path = filedialog.askopenfilename(filetypes=[('Python Files', '*.py')])  # Open file dialog
    if file_path:
        print(f"Selected file: {file_path}")
        instance = CodeTranslatorPy2To3([file_path])
        instance.initialize()
        instance.process_modules()


class CodeTranslatorPy2To3:
    def __init__(self, file_paths=None) -> typing.NoReturn:
        ...
        if self.file_paths:
            for file_path in self.file_paths:
                if file_path.endswith('.py'):
                    self.moduleNameSet.add(file_path)
        else:
...
def select_file():
    """
    A function that opens a file dialog and allows the user to select a Python file.
    """
    ...
    if file_path:
        # If yes, then process the selected file
        instance.process_single_file(file_path)
...
if __name__ == '__main__':
    ...
    root = Tk()
    root.title('Python 2 to 3 Code Translator')

    # Add a label
    label = Label(root, text="Choose a Python file to translate from Python 2 to Python 3")
    label.pack(pady=20)
