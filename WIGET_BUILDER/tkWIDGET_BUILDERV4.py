...
class CodeEditorApp:
    ...
    def setup_ui(self):
        ...
        widget_list=["Entry", "Button", "ComboBox", "CheckButton", "Scale_H",
                                 "Scale_V", "Label",
                                 "Spinbox", "RadioButton", "ListBox", "Text", "Canvas"]
       
        tk.Label(selection_frame, text="Widget Type:").grid(row=0, column=0, sticky="w", pady=2)
        self.widget_type_combo = ttk.Combobox(selection_frame, values = widget_list
        )
        ...
        clear_preview_button.grid(row=12, column=0, pady=10)
        ...
        # Variable Selection
        tk.Label(selection_frame, text="Variable Type:").grid(row=8, column=0, sticky="w", pady=2)
        self.var_type_combo = ttk.Combobox(
            selection_frame,
            values=["None", "StringVar", "IntVar", "DoubleVar", "BooleanVar"]
        )
        self.var_type_combo.grid(row=8, column=1, sticky="ew", pady=2)
        self.var_type_combo.set("None")  # Default is None (No variable binding)

        # Checkbox to enable variable usage
        self.use_var = tk.BooleanVar(value=False)
        self.var_checkbox = ttk.Checkbutton(
            selection_frame, text="Use Variable", variable=self.use_var
        )
        self.var_checkbox.grid(row=9, column=0, columnspan=2, pady=2)
    ...
    def generate_code(self):
        ...
        # Check if a variable should be used
        use_variable = self.use_var.get()
        var_type = self.var_type_combo.get()

        var_name = None
        var_declaration = ""
        if use_variable and var_type != "None":
            var_name = f"var_{self.current_row}_{self.current_column}"
            var_declaration = f"{var_name} = tk.{var_type}()\n"
        ...
        elif widget_type == "Scale_H":
             code = f'{widget_var_name} = tk.Scale(root, from_=0, to=100, orient="horizontal")\n'
             code += f'{widget_var_name}.grid(row={self.current_row}, column={self.current_column}, padx=5, pady=5, sticky="ew")\n'
             widget = tk.Scale(self.preview_area, from_=0, to=100, orient="horizontal")

        elif widget_type == "Scale_V":
             code = f'{widget_var_name} = tk.Scale(root, from_=0, to=100, orient="vertical")\n'
             code += f'{widget_var_name}.grid(row={self.current_row}, column={self.current_column}, padx=5, pady=5, sticky="ns")\n'
             widget = tk.Scale(self.preview_area, from_=0, to=100, orient="vertical")

            
        elif widget_type == "RadioButton":
             radio_var_name = f"radio_var_{self.widget_count}"
             var_code = f"{radio_var_name} = tk.IntVar()\n"
             code = f'{widget_var_name} = tk.Radiobutton(root, text="{text}", variable={radio_var_name}, value={self.widget_count})\n'
             code += f'{widget_var_name}.grid(row={self.current_row}, column={self.current_column}, padx=5, pady=5, sticky="nsew")\n'
             widget = tk.Radiobutton(self.preview_area, text=text, variable=tk.IntVar(), value=self.widget_count)
